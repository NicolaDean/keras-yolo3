...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    box_xy = (tf.math.sigmoid(feats[..., :2]) + grid) / tf.cast(grid_shape[...,::-1], feats.dtype)
    box_wh = tf.math.exp(feats[..., 2:4]) * tf.cast(anchors_tensor,feats.dtype) / tf.cast(input_shape[...,::-1],feats.dtype)
...
def yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape):
    ...
    new_shape = tf.round(image_shape * tf.math.reduce_min(input_shape/image_shape))
    ...
    #warning(f"{boxes.shape}")

    #boxes *= tf.concat([image_shape, image_shape],-1)
    return scale_boxes(boxes,image_shape)
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False,custom_input_format=False):
    ...
    for l in range(num_layers):
        ...
        raw_true_wh = K.switch(object_mask, raw_true_wh, tf.zeros_like(raw_true_wh)) # avoid log(0)=-inf
        ...
        xy_loss = tf.math.reduce_sum(xy_loss) / mf
        wh_loss = tf.math.reduce_sum(wh_loss) / mf
        confidence_loss = tf.math.reduce_sum(confidence_loss) / mf
        class_loss = tf.math.reduce_sum(class_loss) / mf
        loss += xy_loss + wh_loss + confidence_loss + class_loss
        if print_loss:
            loss = tf.Print(loss, [loss, xy_loss, wh_loss, confidence_loss, class_loss, tf.math.reduce_sum(ignore_mask)], message='loss: ')
    return loss
